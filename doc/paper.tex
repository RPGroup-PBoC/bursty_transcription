...
\text{reduce} the transcription rate rather than setting it to zero.
...
This is not wrong, but it is not the full story. The model's posterior is tightly constrained, but is it a good generative model? In other words, does the model generate data that ``look like'' our actual data, and is it therefore plausible that the model captures the important features of the data generating process? This intuitive notion can be codified with \textit{posterior predictive checks}, or PPCs, and we will see that this simple Poisson model fails badly.
\begin{figure}%[h!]
\centering
...
\mmnote{Outline:
\begin{itemize}
\item Poisson model. Write Bayes, briefly explain conjugacy and it's
convenience. immediately write down posterior, make Gaussian
approx. Explain PPC sampling (actually PP dist is analytically
known but you still have to draw RNGs from it. Also this obscures
the intuitive meaning of PPC sampling from posterior and
generating synthetic data, so I think it's more useful to just
generate PP samples from posterior draws, this gives a more
intuitive apples-to-apples comparison w/ model 5).
