...
\caption{
\mmnote{This figure wouldn't appear in this section, instead
just a reference to the cartoon so the reader knows
what cartoon model we're doing all this math for.}
}
\end{figure}
...
A schematic of this model is shown in Fig.\ 2A.
...
evaluating the likelihood defined by~\eref{eq:p_m_bursty+rep} $\sim50$ times
...
model,~\eref{eq:p_m_bursty+rep}, we need to evaluate hypergeometric functions
...
relation~\eref{eq:chosen_rec_rel} if the recursion is run
...
$p(m)$ in~\eref{eq:p_m_bursty+rep} is overflow of the gamma functions.
