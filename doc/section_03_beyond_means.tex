...
in the previous section by examining the models one by one. \mrm{In particular
we will focus on the Fano factor, defined as the variance/mean. This metric
serves as a discriminatory tool from the null model that the steady-state mRNA
...
The left column depicts various plausible models for the dynamics of
...
cannot produce the FISH data observed in~\cite{Jones2014} without introducing
additional assumptions and model complexity.}
...
As shown in Appendix~\ref{sec:unreg_promoters}, the Fano factor in this model is
given by
\begin{align}
    \nu = 1 -
        \frac{r\rate{k}{P}{+}}
...
population of cells.  Indeed, as shown in Appendix~\ref{sec:unreg_promoters}, a
...
Appendix~\ref{sec:unreg_promoters} takes the form
\begin{align}
\begin{split}
...
\end{split}
\end{align}
\mrm{notice I changed $k^+$ to $k_i$ to be consistent with the figure.}
