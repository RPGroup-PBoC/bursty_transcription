...
details to the appendix \mrm{cite specific appendix section}. What we will find
is that all cartoons collapse to a single master curve, shown in
\fig{fig1:means_cartoons}(D), which contains just two parameters. We label the
...
The derivation of this result is deferred to the appendix \mrm{cite specific 
section}.
...
out in the appendix \mrm{cite section} where we find
\begin{equation}
\langle m\rangle = \frac{r}{\gamma}
        \frac{k_R^- k_P^+ k_O}
...
\subsubsection{Summary of Results Ccross models for Fold-Changes in Mean
Expression}
...
(looking ahead, this is equivalent to model 4 in~\fig{fig:constit_cartoons}), to
