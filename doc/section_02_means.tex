...
level in the absence of repressor~\mmnote{look up cite}. Computing fold-changes
...
cartoons collapse to a single master curve, shown in \fig{fig1:means_cartoons}(C),
...
in~\fig{fig1:means_cartoons}.  Thus far,
...
\end{equation}
again matching the master curve of~\fig{fig1:means_cartoons}(C) with $\rho=1$.
...
&=      \left(1 + \frac{k_R^+}{k_R^-}
        ...
        \right)^{-1},
\end{align}
which follows the master curve of~\fig{fig1:means_cartoons}(C) with $\rho = 1 + k_P^+/(k_P^- + r)$ as claimed.
...
\right)^{-1},
\end{align}
surprisingly reducing to the master curve of~\fig{fig1:means_cartoons}(C) once again, with $\rho = 1 +
...
the means giving
in~\fig{fig1:means_cartoons}(C) so long as the repressor binding is exclusive with
...
\subsubsection{Summary of Results Ccross models for Fold-Changes in Mean Expression}
...
expression levels? The key outcome of our analysis is the existence of a master curve
 shown in \fig{fig1:means_cartoons}(C) to which the fold-change predictions of all the
...
Why does the master curve of~\fig{fig1:means_cartoons}(C) exist at all? This
...
model in~\fig{fig1:means_cartoons}. In that picture, the weak promoter
...
fitting nonequilibrium model 4  in~\fig{fig1:means_cartoons}, with zero
repressor (looking ahead, this is equivalent to model 4
in~\fig{fig:constit_cartoons}), to FISH data from~\cite{Brewster2014}. This
...
agree with repressor binding and unbinding rates. In other words, the equilbrium
and nonequilibrium definitions of $\Delta F_R$ must agree, meaning
\begin{equation}
\Delta F_R = \beta\Delta\epsilon_R - \log(R/N_{NS})
        = - \log(k_R^+/k_R^-),
\end{equation}
must hold, where $\beta\Delta\epsilon_R$ is inferred from the master curve fit
to fold-change measurements, and $k_R^+$ and $k_R^-$ are inferred in some
