...
production of mRNA can only happen when the promoter is in the transcriptionally
active state.}
\label{fig:two_state}
\end{figure}
...
interval such that only one of the possible reactions take place. For the
example that we will workout here in detail we chose the two-state stochastic
...
Since mRNA production can only take place on the transcriptionally active state
...
\begin{itemize}
    ...
    \item Transition from the repressor bound state $R$ to the active state $A$.
\end{itemize}
...
\begin{itemize}
    ...
    \item Transition from the active state $A$ to the repressor bound state $R$.
\end{itemize}
The mRNA production does not depend on the current number of mRNAs, therefore
these state transitions happen with probability $r\Delta t$. The same is true
for the promoter state transitions, each happen with probability $k_R^\pm \Delta
t$. As for the mRNA degradation events does must depend on the current number of
mRNA molecules. Each molecule has a constant probability $\gamma \Delta t$ of 
...
being in state $R$ to begin with. It is through this term that both probability
distribution functions are coupled. An equivalent equation can be written for
...
\end{aligned}
\end{equation}
In the next section we will write these couple ODEs in a more compat form using
matrix notation.
...
models, the natural mathematization of their cartoons is as a chemical master
