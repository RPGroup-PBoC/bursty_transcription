...
have different strengths. This model insists that the ``true'' steady-state mRNA
...
Fano factor. For \mrm{challenging} the ``universal noise curve'' from So et.\
al.~\cite{So2011}, this picture was excellent. It is appealing in its
...
many interesting questions, for instance in the recent
work~\cite{Razo-Mejia2020}, knowledge of means and variances alone is
...
\mrm{I added the word challenging instead of disproving because of the 
following comment from reviewer 1. But maybe more needs to be discussed about 
this?}

\mrm{
    “disproving the universal noise curve from So et. al. [59]” (page 18). How was the universal noise curve disproved? Universal noise/mean relations, similar to those reported in ref \#59, were repeatedly observed, in both bacteria and eukaryotes (see, e.g., PMID: 24311680, PMID: 25858977). Moreover, later reports showed that the trend is observed even when accounting for gene dosage effects (PMID: 26965629). Thus, the statement above requires clarification.
}
...
of these unregulated promoters --which only differ on the promoter sequence--
...
\mrm{I like this paragraph's philosphy, but I don't know if it should be cut out 
for the sake of shrinking the paper further.}
...
e.g.,~\cite{Golding2005,Chong2014,Sevier2016,Sepulveda2016}. Let us make this
correspondence more precise. The mean dwell time in the active state is $1/k^-$.
While in this state, transcripts are produced at a rate $r$ per unit time. So on
average, $r/k^-$ transcripts are produced before the system switches to the
inactive state. Once in the inactive state, the system dwells there for an
average time $1/k^+$ before returning to the active state and repeating the
...
\mrm{Another great paragraph that could potentially be removed.}
